...
/**
 * Branch-only proof of concept.
 *
 * 기존 V2처럼 1차 XLSX 생성 -> 다시 열기 -> 2차 XLSX 생성을 하지 않고,
 * 원본 템플릿을 한 번 열어서 DB 값을 모두 반영한 뒤 마지막에 한 번만 write 한다.
 */
class StatementWorkbookOnePassPrototypeService {
    ...
    StatementWorkbookOnePassPrototypeService(
    ...
    StatementWorkbookResult generate(
    ...
    private InputStream openTemplate(MultipartFile templateFile) throws IOException {
        ...
        if (!resource.exists()) {
            throw new IllegalArgumentException(
                    "기본 template.xlsx 파일을 찾을 수 없습니다. src/main/resources/template.xlsx를 확인해주세요."
            );
        }
