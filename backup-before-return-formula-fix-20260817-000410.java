...
public class StatementWorkbookService {
    ...
    public StatementWorkbookResult generate(
        ...
        try (
            ...
            for (
                    int index =
                            sheetIndexesToRemove.size() - 1;
                    index >= 0;
                    index--
            ) {
                workbook.removeSheetAt(
                        sheetIndexesToRemove.get(
                                index
                        )
                );
            }
    ...
    private String normalizeTemplateItem(
        ...
        /*
         * 새 template1.xlsx의 팔공식품 시트에서
         * F열 '일소'는 실제 수식상 회수통입니다.
         * G열 합계식도 -F*3000 형태입니다.
         */
        if ("일소".equals(label)) {
            return "회수통";
        }
    ...
    private void fillSheet(
        ...
        Map<LocalDate, BigDecimal>
                ...
                createReturnContainerAmountByDate(
                        items
                );
    ...
    private void writePeriod(
        ...
        /*
         * template1.xlsx는 A3에 기존 #REF!가 남아 있어
         * 해당 셀을 정산기간 문자열로 교체합니다.
         *
         * 이전 template.xlsx 구조도 헤더가 32행이면
         * 기존 A7 위치를 계속 사용합니다.
         */
        cell.setCellValue(
                periodText
    ...
    private void addMissingTemplateWarnings(
        ...
        for (
                Map.Entry<String, List<SalesItemEntity>>
                        entry
    ...
    private void applyReturnContainerAmountToFormula(
        ...
        String returnReference =
                returnCell.getAddress()
                        .formatAsString();

        List<Cell> changedFormulaCells =
                new ArrayList<>();

        Cell nearestFormulaCell =
                null;
        ...
        for (
            ...
            String cleanedFormula =
                    removeReturnContainerTerm(
                            originalFormula,
                            returnReference
                    );

            if (
                    !cleanedFormula.equals(
                            originalFormula
                    )
            ) {
                cell.setCellFormula(
    ...
    private void recalculateFormulas(
        ...
        try {
            ...
            /*
             * 일부 오래된 Excel 수식은 POI가
             * 계산하지 못할 수 있습니다.
             * Excel을 열 때 재계산하도록 설정했으므로
             * 파일 생성은 계속합니다.
             */
        }
    }
